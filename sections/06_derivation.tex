...
We adopt the v0.3.1 convention that the Enchan field $S$ is \textbf{dimensionless} and is interpreted as an effective time-dilation factor.
...
\label{eq:V0_anchor}
\end{equation}
where $\alpha_c$ is a dimensionless geometric factor.  This should be read as a \textbf{minimal ansatz}: it is the simplest local energetic scaling constructed from $\Sigma_{\rm b}$ and $G$.
...
Ch.~\ref{sec:obs_btfr} and Ch.~\ref{sec:obs_rar} (benchmarks) summarize the observed emergence of an acceleration scale $\sim 10^{-10}\,\mathrm{m/s^2}$ from public data.
