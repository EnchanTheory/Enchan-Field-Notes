#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Enchan Core Model Plus (v0.4.0 / Eq. 6.6.2 ONLY)

This module implements the galaxy-calibrated pinning (Phi-screening) mechanism only.
It intentionally excludes any high-acceleration (g-screening) safeguard terms.

Key Features:
1) Eq. 6.6.2 Phi-screening only: S_phi(|Phi|)
2) Fail-Fast: returns NaN for invalid inputs in proxy/screening
3) Dual proxy support:
   - calculate_phi_at_radius: local proxy at r_c (strict)
   - calculate_phi_median_proxy: robust median proxy (NaN-tolerant)

IMPORTANT:
- Do NOT pass g-screening parameters here. If g_c/m/g_bar are passed, this module raises.
- High-acceleration suppression is implemented in: enchan_core_model_g_screening.py
"""

from __future__ import annotations
import numpy as np
from typing import Optional, Union

# =========================================================
# Constants & Defaults
# =========================================================

KPC_TO_M = 3.08567758e19

# Golden Parameters (Optimized from Q1/Q4 Nested CV)
DEFAULT_PHI_C = 40000.0  # (km/s)^2
DEFAULT_N     = 1.0

# =========================================================
# 1. Proxy Calculation (Stateless, NumPy only)
# =========================================================

def calculate_phi_at_radius(
    r_kpc: np.ndarray,
    g_bar: np.ndarray,
    target_r_kpc: float
) -> float:
    """
    Local potential-depth proxy at a specific radius:
        |Phi|(r) ~ V_bar(r)^2  with  V_bar^2 = g_bar(r) * r

    Returns:
        (km/s)^2, or NaN if invalid/out-of-bounds (no extrapolation).
    """
    r = np.asarray(r_kpc, dtype=float)
    g = np.asarray(g_bar, dtype=float)

    if r.size == 0 or r.shape != g.shape:
        return float("nan")

    # Strict finite check (Fail-Fast)
    if not (np.all(np.isfinite(r)) and np.all(np.isfinite(g))):
        return float("nan")

    # Sort for interpolation
    sort_idx = np.argsort(r)
    r_sorted = r[sort_idx]
    g_sorted = g[sort_idx]

    # V^2 = g*r in (m^2/s^2); convert to (km/s)^2 by /1e6
    v2_profile = (g_sorted * (r_sorted * KPC_TO_M)) / 1.0e6

    # No extrapolation
    if target_r_kpc < r_sorted[0] or target_r_kpc > r_sorted[-1]:
        return float("nan")

    return float(np.interp(target_r_kpc, r_sorted, v2_profile))


def calculate_phi_median_proxy(
    r_kpc: np.ndarray,
    g_bar: np.ndarray,
    inner_cut_kpc: Optional[float] = None
) -> float:
    """
    Robust galaxy-wide summary proxy (median) of |Phi| ~ V_bar^2.

    NaN-tolerant: filters invalid points before median.
    """
    r = np.asarray(r_kpc, dtype=float)
    g = np.asarray(g_bar, dtype=float)

    if r.size == 0 or r.shape != g.shape:
        return float("nan")

    if inner_cut_kpc is not None:
        mask = r >= inner_cut_kpc
        r = r[mask]
        g = g[mask]

    if r.size == 0:
        return float("nan")

    valid = np.isfinite(r) & np.isfinite(g)
    r = r[valid]
    g = g[valid]
    if r.size == 0:
        return float("nan")

    v2 = (g * (r * KPC_TO_M)) / 1.0e6
    return float(np.median(v2))

# =========================================================
# 2. Eq. 6.6.2 Phi-Screening (Pinning) ONLY
# =========================================================

def get_phi_screening_factor(
    phi_val: Union[float, np.ndarray],
    phi_c: float = DEFAULT_PHI_C,
    n: float = DEFAULT_N
) -> Union[float, np.ndarray]:
    """
    Eq. 6.6.2:
        S_phi(|Phi|) = [1 + (|Phi|/Phi_c)^n]^{-1}

    Returns NaN if inputs are invalid.
    """
    if n <= 0:
        raise ValueError(f"Phi-screening index 'n' must be > 0, got {n}")

    if phi_val is None:
        return float("nan")

    if not np.all(np.isfinite(phi_val)):
        return float("nan")

    if phi_c is None or phi_c <= 0:
        # "No screening" (explicit)
        return np.ones_like(phi_val, dtype=float) if isinstance(phi_val, np.ndarray) else 1.0

    return 1.0 / (1.0 + (np.abs(phi_val) / float(phi_c)) ** float(n))


def apply_screening(
    a0_free: Union[float, np.ndarray],
    phi_val: Optional[Union[float, np.ndarray]] = None,
    phi_c: float = DEFAULT_PHI_C,
    n: float = DEFAULT_N,
    # Guard rails: reject g-screening args here
    g_bar: Optional[Union[float, np.ndarray]] = None,
    g_c: Optional[float] = None,
    m: Optional[float] = None,
) -> Union[float, np.ndarray]:
    """
    Apply Eq. 6.6.2 (Phi-screening) to a0_free:
        a0_eff = a0_free * S_phi(|Phi|)

    IMPORTANT:
    - This module is 6.6.2-only. If g_bar/g_c/m are provided, we raise to prevent mixing.
    """
    if g_bar is not None or g_c is not None or m is not None:
        raise ValueError(
            "g-screening arguments detected (g_bar/g_c/m). "
            "Use enchan_core_model_g_screening.py (Eq. 6.6.3) for high-acceleration suppression."
        )

    if not np.all(np.isfinite(a0_free)):
        return float("nan")

    if phi_val is None:
        return float("nan")

    s_phi = get_phi_screening_factor(phi_val=phi_val, phi_c=phi_c, n=n)
    return a0_free * s_phi
