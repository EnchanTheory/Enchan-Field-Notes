...
This folder contains reproducible, public-data benchmarks used in *Enchan Field Notes v0.3.2*.
...
- Important: several steps use an effective closure (“Enchan quadrature law”) as a working hypothesis:
  ...
  In v0.3.2, the field-theoretic derivation of this closure and of $a_0$ is the theory-side goal.
...
### 1) BTFR benchmark (and implied $a_0$ distribution)

Computes BTFR points, a simple log–log fit, and the implied per-galaxy estimate:
...
Uses SPARC `Rotmod_LTG.zip` to compute $g_{\rm bar}(r)$ and compare to observed accelerations,
...
### 3) Rotation-curve prediction (shape stress test)
...
### 4) Exploration: $a_0$ vs surface-brightness proxy (v0.3.2)
...
### 5) Mathematical consistency check for the transition function (v0.3.2)

Numerically checks that the candidate transition function $\mu(x)$ used in the note is
...
Note: ensure the figure filename in this README matches the one produced by the script.

---

## Recommended run order (quickstart)

1. Put `BTFR_Lelli2019.mrt` and `Rotmod_LTG.zip` next to these scripts (or pass explicit paths).
...
5. Run the correlation script to explore the Chapter 6 anchor ansatz.
